...
# Display summaries as  tables
knitr::kable(height_summary, 
             caption = "Table 1: Height Statistics by Species Group",
             digits = 1)

knitr::kable(hair_summary %>% head(6), 
             caption = "Table 2: Distribution of Hair Colors (Top 6)",
             digits = 3)

```

```{r}
#| label: create-beautiful-visuals
...
# A visual showing height distribution for different species groups
sw.height <- sw.optional
  ggplot(sw.optional, aes(x = species_group, y = height_cm, fill = species_group)) +
  geom_boxplot() +
  ...
  labs(
    title = "Height Distribution Across Species Groups",
    x = "Species Group",
    y = "Height (cm)",
    fill = "Species Group"
  ) 
